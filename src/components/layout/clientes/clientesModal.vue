               ...
               <div class="modal-dialog">
                   <div class="modal-content">
                       ...
                       <div class="modal-body">
                           ...
                           <p>{{$t("Componentes.Modal.Solicitar_informacion")}}</p>
                           <h3>{{cliente}}</h3>
                            <fieldset :disabled="loading">
                           <form method="post" @submit.prevent="enviarMsg">
                               <div class="form-group"><input class="form-control" required type="text" v-model="ctaName" :placeholder="$t('Componentes.InputsPlaceholder.Nombre')"></div>
                               <div class="form-group"><input class="form-control" required type="tel" v-model="ctaPhone" :placeholder="$t('Componentes.InputsPlaceholder.Telefono')"></div>
                               <div class="form-group"><input class="form-control" type="email" v-model="ctaMail" :placeholder="$t('Componentes.InputsPlaceholder.Email')"></div>
        ...
        methods: {
            ...
            enviarMsg(){
                ...
                let bodyFormData = new FormData();
                    ...
                    bodyFormData.append('email', this.ctaMail);
                    bodyFormData.append('cliente', this.cliente);

                this.axios.post("contact-form-7/v1/contact-forms/74/feedback", bodyFormData, {
                headers: headers
                        })
                .then((response)=> {
                        this.success = true;                            
